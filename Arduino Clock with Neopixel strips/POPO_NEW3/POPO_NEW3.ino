...
void setup() {
  ...
  int hourSetup = now.hour(), minuteSetup = now.minute();
  int tpos = 0;
  update(hourSetup, minuteSetup);
  while (1) {
  ...
  Clock.setHour(hourSetup);
  Clock.setMinute(minuteSetup);

  attachInterrupt(digitalPinToInterrupt(btn_1), btnPress1, LOW);
  attachInterrupt(digitalPinToInterrupt(btn_2), btnPress2, LOW);
  attachPCINT(digitalPinToPCINT(btn_3), btnPress3, LOW);
...
void loop() {
  DateTime now = myRTC.now();
  update(now.hour(), now.minute());
  delay(100);
}

void update(byte getHo, byte getMin) {
  /*
    Serial.print(getHo);
    Serial.print(" : ");
    Serial.println(getMin);
  */
  byte actualHour = getHo;
  byte actualMinute = getMin;
