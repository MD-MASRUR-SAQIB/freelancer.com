...
void MouseReportParser::Parse(USBHID *hid __attribute__((unused)), bool is_rpt_id __attribute__((unused)), uint8_t len __attribute__((unused)), uint8_t *buf) {
		...
		if (prevState.mouseInfo.bmSideButtonUp == 0 && pmi->bmSideButtonUp == 1)
                OnSideButtonUpDown(pmi);

        if (prevState.mouseInfo.bmSideButtonUp == 1 && pmi->bmSideButtonUp == 0)
                OnSideButtonUpUp(pmi);
			
		if (prevState.mouseInfo.bmSideButtonDown == 0 && pmi->bmSideButtonDown == 1)
                OnSideButtonDownDown(pmi);

        if (prevState.mouseInfo.bmSideButtonDown == 1 && pmi->bmSideButtonDown == 0)
                OnSideButtonDownUp(pmi);

        if (prevState.mouseInfo.dX != pmi->dX || prevState.mouseInfo.dY != pmi->dY || prevState.mouseInfo.Wheel != pmi->Wheel)
                OnMouseMove(pmi);
		...
		if(prevState.mouseInfo.bmSideButtonUp != pmi->bmSideButtonUp) {
                if(pmi->bmSideButtonUp) {
                        OnSideButtonUpDown(pmi);
                } else {
                        OnSideButtonUpUp(pmi);
                }
		...
		if(prevState.mouseInfo.bmSideButtonDown != pmi->bmSideButtonDown) {
                if(pmi->bmSideButtonDown) {
                        OnSideButtonDownDown(pmi);
                } else {
                        OnSideButtonDownUp(pmi);
                }
